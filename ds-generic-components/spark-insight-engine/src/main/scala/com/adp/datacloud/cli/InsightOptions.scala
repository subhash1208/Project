...
case class InsightConfig(
    ...
    applicationName: String                          = "ADP Insights Builder",
...
object InsightOptions {

  def parse(args: Array[String]) = {
    val parser = new scopt.OptionParser[InsightConfig]("Insight Engine") {
      head("InsightEngine", this.getClass.getPackage().getImplementationVersion())

      override def showUsageOnError = true

      opt[String]('a', "application-name")
      ...
      opt[String]('p', "input-partition-spec")
        .unbounded()
        .action((x, c) => c.copy(inputPartitionSpec = c.inputPartitionSpec ++ List(x)))
        .text("Optional partition spec to filter the input sql with")

      opt[String]('c', "comparison-spec")
        .action((x, c) => c.copy(compare = Some(x)))
        .text("Compare Either of 1) Fully qualified table or 2) Sql file")

      opt[String]('s', "compare-partition-spec")
        .unbounded()
        .action(
          (x, c) => c.copy(comparePartitionSpec = c.comparePartitionSpec ++ List(x)))
        .text("Optional partition spec to filter the compare sql with")
      ...
      opt[String]('o', "output-db")
        .required()
        .action((x, c) => c.copy(outputDB = x))
        .text("Hive DB where output tables should be saved. Ignored if hdfs-output-path is provided")
      ...
      opt[Double]("min-pctg-diff")
        .action((x, c) => c.copy(minimumPercentageDifference = x))
        .text(
          "Minimum value of percentage difference for Diff based insights. Default is 5%")
      ...
      note("ADP Insight Generation Framework. \n")

    }

    // parser.parse returns Option[C]
    parser.parse(args, InsightConfig()).get
