SELECT 
      ...
      rs_pr_prac.total_earnings/rs_pr_prac.count_distinct_person AS average_earnings,
  ...
  LEFT OUTER JOIN ${hiveconf:__RO_BLUE_RAW_DB__}.dwh_t_dim_fiscal_clnts fscl
    ...
    AND fscl.dmn_ky = 3
WHERE 
--rs_pr_prac.d_flsa_stus_cd IS NULL
--rs_pr_prac.d_gndr_cd IS NULL
rs_pr_prac.d_job_cd IS NULL
--AND rs_pr_prac.d_reg_temp_cd IS NULL
AND rs_pr_prac.d_work_cntry_cd IS NULL
AND rs_pr_prac.d_work_state_cd IS NULL
AND rs_pr_prac.d_hr_orgn_id IS NULL
--AND rs_pr_prac.d_pay_rt_type_cd IS NULL
--AND rs_pr_prac.headcount >= 50
--AND rs_pr_prac.d_full_tm_part_tm_cd IS NULL
...
AND (
    ...
    (
        ...
        COALESCE(rs_pr_prac.mnth_cd, date_format(add_months(current_date, -1), 'yyyymm')) = date_format(add_months(current_date, -1), 'yyyymm')
        -- This is previous quarter = cast(year(add_months(current_date, -3)) *10 + ceil(month(add_months(current_date, -3))/3) as string)
        AND 
            COALESCE(CASE WHEN rs_pr_prac.mnth_cd IS NULL THEN rs_pr_prac.qtr_cd ELSE NULL END, 
        ...
        AND rs_pr_prac.yr_cd >= date_format(add_months(current_date, -12), 'yyyy')
    )
  )