...
FROM 
    ...
    -- facts
    tm_mngr.absence_ratio,
    cast(tm_mngr.absence_hrs_event_count as double) as absence_ratio_events,
    --tm_mngr.leave_ratio,
    tm_mngr.overtime_ratio,
    cast(tm_mngr.ot_hrs_event_count as double) as overtime_ratio_events,
    --tm_mngr.total_hrs,
...
FROM
    ${hiveconf:__GREEN_MAIN_DB__}.emi_cube_tm_tf_manager tm_mngr
...
FROM
    ${hiveconf:__BLUE_MAIN_DB__}.emi_prep_tm_tf_etime_mngr_agg tm_mngr
INNER JOIN (SELECT clnt_obj_id, mngr_pers_obj_id, yr_cd, qtr_cd, mnth_cd,environment, num_employees AS tot_rpt_headcount 
            FROM ${hiveconf:__BLUE_MAIN_DB__}.emi_prep_tm_tf_etime_mngr_agg 
