SELECT 
      ...
      rs_prac.qtr_cd,
      rs_prac.mnth_cd,
      --NULL AS d_flsa_stus_cd,
      --NULL AS d_gndr_cd,
      cast(NULL AS STRING) AS d_job_cd,
      --NULL AS d_reg_temp_cd,
      cast(NULL AS STRING) AS d_work_loc_cd,
      --NULL AS d_work_city_cd,
      cast(NULL AS STRING) AS d_work_cntry_cd,
      cast(NULL AS STRING) AS d_hr_orgn_id,
      --NULL AS d_pay_rt_type_cd,
      cast(NULL AS STRING) AS d_work_state_cd,
      --NULL AS d_full_tm_part_tm_cd,
      cast(NULL AS STRING) AS d_trmnt_rsn,
 ...
 INNER JOIN (SELECT clnt_obj_id, yr_cd, qtr_cd, mnth_cd,environment, num_employees AS tot_rpt_headcount
   ...
   AND NVL(rs_prac.yr_cd, "UNKNOWN") = NVL(maxhc.yr_cd, "UNKNOWN")
   AND NVL(rs_prac.qtr_cd, "UNKNOWN") = NVL(maxhc.qtr_cd, "UNKNOWN")
   AND NVL(rs_prac.mnth_cd, "UNKNOWN") = NVL(maxhc.mnth_cd, "UNKNOWN")
   AND rs_prac.environment = maxhc.environment
   --AND rs_prac.db_schema = maxhc.db_schema
 INNER JOIN ${hiveconf:__GREEN_MAIN_DB__}.emi_cube_hr_wef_practitioner rs_hr_wef
 ...
 INNER JOIN ${hiveconf:__BLUE_MAIN_DB__}.emi_prep_retn_prac rs_retn
  ...
  LEFT OUTER JOIN (SELECT DISTINCT environment,mnth_cd, cast(mnth_seq_nbr as int) as mnth_seq_nbr
  ...
  LEFT OUTER JOIN (SELECT DISTINCT environment,qtr_cd,  cast(qtr_seq_nbr as int) as qtr_seq_nbr
  ...
  LEFT OUTER JOIN ${hiveconf:__RO_BLUE_RAW_DB__}.dwh_t_dim_fiscal_clnts fscl
    ...
    AND fscl.dmn_ky = 2
 WHERE 
 --rs_prac.d_flsa_stus_cd IS NULL
 --rs_prac.d_gndr_cd IS NULL
 rs_prac.d_job_cd IS NULL
 --AND rs_prac.d_reg_temp_cd IS NULL
 AND rs_prac.d_work_cntry_cd IS NULL
 AND rs_prac.d_hr_orgn_id IS NULL
 --AND rs_prac.d_pay_rt_type_cd IS NULL
 AND rs_hr_wef.d_trmnt_rsn IS NULL 
 --AND rs_prac.d_full_tm_part_tm_cd IS NULL
 AND rs_prac.period_end_active_pers_cnt != 0
 ...
 AND (
    ...
    (
        ...
        COALESCE(rs_prac.mnth_cd, date_format(add_months(current_date, -1), 'yyyymm')) = date_format(add_months(current_date, -1), 'yyyymm')
        -- This is previous quarter = cast(year(add_months(current_date, -3)) *10 + ceil(month(add_months(current_date, -3))/3) as string)
        AND 
            COALESCE(CASE WHEN rs_prac.mnth_cd IS NULL THEN rs_prac.qtr_cd ELSE NULL END, 
        ...
        AND rs_prac.yr_cd >= date_format(add_months(current_date, -12), 'yyyy')
    )
