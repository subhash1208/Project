...
FROM
    ...
    -- facts
    tm_mngr.exception_hours,
    tm_mngr.exception_count,
    cast(tm_mngr.exception_count as double) as exception_count_events,
...
FROM
    ${hiveconf:__GREEN_MAIN_DB__}.emi_cube_tm_tf_excp_manager tm_mngr
...
FROM
    ${hiveconf:__BLUE_MAIN_DB__}.emi_prep_tm_tf_excp_etime_mngr_agg tm_mngr
INNER JOIN (SELECT clnt_obj_id, mngr_pers_obj_id, yr_cd, qtr_cd, mnth_cd,environment, SUM(num_employees) AS tot_rpt_headcount 
               FROM ${hiveconf:__BLUE_MAIN_DB__}.emi_prep_tm_tf_excp_etime_mngr_agg 
              WHERE job_cd is null and hr_orgn_id is null and work_cntry_cd is null and work_state_cd is null and work_loc_cd is null and clndr_wk_cd is null
              GROUP BY environment,clnt_obj_id,mngr_pers_obj_id,yr_cd,qtr_cd,mnth_cd
              ) maxhc
ON tm_mngr.clnt_obj_id = maxhc.clnt_obj_id 
AND tm_mngr.mngr_pers_obj_id <=> maxhc.mngr_pers_obj_id
AND tm_mngr.yr_cd <=> maxhc.yr_cd
AND tm_mngr.qtr_cd <=> maxhc.qtr_cd
AND tm_mngr.mnth_cd <=> maxhc.mnth_cd
AND tm_mngr.environment = maxhc.environment)b
)tm_mngr
