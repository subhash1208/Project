SELECT 
       ...
       -- facts
       --rs_mgr.female_percentage,
       rs_mgr.period_end_active_pers_cnt AS headcount,
       cast(rs_mgr.period_end_active_pers_cnt as double) as headcount_events,
       maxhc.tot_rpt_headcount,
       ROUND((rs_mgr.num_employees / maxhc.tot_rpt_headcount)*100,1) AS percentage_headcount,

       --rs_mgr.inactive_headcount,
       --rs_mgr.parttime_headcount,
       --rs_mgr.temp_headcount,
       ...
       100 * rs_hr_wef.num_terminations / rs_mgr.headcount AS turnover_rate,
       cast(rs_hr_wef.num_terminations as double) as turnover_rate_events,
       100 * rs_hr_wef.num_voluntary_terminations / rs_mgr.headcount AS voln_turnover_rate,
       cast(rs_hr_wef.num_voluntary_terminations as double) as voln_turnover_rate_events,
       100 * rs_hr_wef.num_newhire_turnovers /  rs_mgr.headcount AS newhire_turnover_rate,
       cast(rs_hr_wef.num_newhire_turnovers as double) as newhire_turnover_rate_events,
       ROUND((100 * rs_mgr.leave_person_days / rs_mgr.period_end_active_pers_cnt),2) AS leave_percentage,
       cast(rs_mgr.leave_person_count as double) as leave_percentage_events,
       --rs_mgr.compa_ratio,
       --rs_mgr.span_of_control,
       ROUND(rs_mgr.average_tenure, 2) AS average_tenure,
       ...
       --rs_mgr.retention_rate,
       --rs_mgr.period_strt_person_cnt AS retn_period_strt_person_cnt,
       --rs_mgr.period_end_person_cnt AS retn_period_end_person_cnt,
  ...
  FROM ${hiveconf:__GREEN_MAIN_DB__}.emi_cube_hr_waf_manager rs_mgr
 INNER JOIN (SELECT clnt_obj_id, mngr_pers_obj_id, yr_cd, qtr_cd, mnth_cd,environment,num_employees AS tot_rpt_headcount 
              ...
              WHERE d_job_cd IS NULL AND d_hr_orgn_id IS NULL AND d_work_cntry_cd IS NULL
            ) maxhc
 ...
 LEFT OUTER JOIN ${hiveconf:__GREEN_MAIN_DB__}.emi_cube_hr_wef_manager rs_hr_wef
    ON rs_mgr.clnt_obj_id <=> rs_hr_wef.clnt_obj_id
   AND rs_mgr.mngr_pers_obj_id <=> rs_hr_wef.mngr_pers_obj_id
   AND rs_mgr.yr_cd <=> rs_hr_wef.yr_cd
   AND rs_mgr.qtr_cd <=> rs_hr_wef.qtr_cd
   AND rs_mgr.mnth_cd <=> rs_hr_wef.mnth_cd
   AND rs_mgr.environment <=> rs_hr_wef.environment
   --AND rs_mgr.d_gndr_cd <=> rs_hr_wef.d_gndr_cd
   AND rs_mgr.d_job_cd <=> rs_hr_wef.d_job_cd
   AND rs_mgr.d_hr_orgn_id <=> rs_hr_wef.d_hr_orgn_id
   AND rs_mgr.d_work_loc_cd <=> rs_hr_wef.d_work_loc_cd
   --AND rs_mgr.d_work_city_cd <=> rs_hr_wef.d_work_city_cd
   AND rs_mgr.d_work_state_cd <=> rs_hr_wef.d_work_state_cd
   AND rs_mgr.d_work_cntry_cd <=> rs_hr_wef.d_work_cntry_cd
   --AND rs_mgr.db_schema = rs_hr_wef.db_schema
 LEFT OUTER JOIN ${hiveconf:__BLUE_MAIN_DB__}.emi_prep_retn_mngr rs_retn
    ON rs_mgr.clnt_obj_id <=> rs_retn.clnt_obj_id
   AND rs_mgr.mngr_pers_obj_id <=> rs_retn.mngr_pers_obj_id
   AND rs_mgr.yr_cd <=> rs_retn.yr_cd
   AND rs_mgr.qtr_cd <=> rs_retn.qtr_cd
   AND rs_mgr.mnth_cd <=> rs_retn.mnth_cd
   AND rs_mgr.environment <=> rs_retn.environment   
   AND rs_mgr.d_job_cd <=> rs_retn.d_job_cd
   AND rs_mgr.d_hr_orgn_id <=> rs_retn.d_hr_orgn_id
   AND rs_mgr.d_work_loc_cd <=> rs_retn.d_work_loc_cd   
   AND rs_mgr.d_work_state_cd <=> rs_retn.d_work_state_cd
   AND rs_mgr.d_work_cntry_cd <=> rs_retn.d_work_cntry_cd
  -- AND rs_mgr.db_schema = rs_retn.db_schema 
  ...
  LEFT OUTER JOIN ${hiveconf:__RO_BLUE_RAW_DB__}.dwh_t_dim_fiscal_clnts fscl
    ...
    AND fscl.dmn_ky = 2
WHERE 
  rs_mgr.yr_cd IS NOT NULL 
  AND rs_hr_wef.d_trmnt_rsn IS NULL
  ...
  AND (
    ...
    (
        COALESCE(rs_mgr.mnth_cd, date_format(add_months(to_date(current_date,'yyyy/MM/dd'), -1), 'yyyyMM')) 
        IN (date_format(add_months(to_date(current_date,'yyyy/MM/dd'), -1), 'yyyyMM'),
            date_format(add_months(to_date(current_date,'yyyy/MM/dd'), -2), 'yyyyMM'),
            date_format(add_months(to_date(current_date,'yyyy/MM/dd'), -13), 'yyyyMM')
        )
        -- This is previous quarter = cast(year(add_months(to_date(current_date,'yyyy/MM/dd'), -3)) *10 + ceil(month(add_months(to_date(current_date,'yyyy/MM/dd'), -3))/3) as string)
        AND COALESCE(CASE WHEN rs_mgr.mnth_cd IS NULL THEN rs_mgr.qtr_cd ELSE NULL END, cast(year(add_months(to_date(current_date,'yyyy/MM/dd'), -3)) *10 + ceil(month(add_months(to_date(current_date,'yyyy/MM/dd'), -3))/3) as string)) 
        IN (
            cast(year(add_months(to_date(current_date,'yyyy/MM/dd'), -3)) *10 + ceil(month(add_months(to_date(current_date,'yyyy/MM/dd'), -3))/3) as string),
            cast(year(add_months(to_date(current_date,'yyyy/MM/dd'), -6)) *10 + ceil(month(add_months(to_date(current_date,'yyyy/MM/dd'), -6))/3) as string),
            cast(year(add_months(to_date(current_date,'yyyy/MM/dd'), -15)) *10 + ceil(month(add_months(to_date(current_date,'yyyy/MM/dd'), -15))/3) as string)
        )
        AND rs_mgr.yr_cd >= date_format(add_months(to_date(current_date,'yyyy/MM/dd'), -24), 'yyyy')
    )
