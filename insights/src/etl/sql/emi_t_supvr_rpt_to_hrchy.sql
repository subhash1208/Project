...
CREATE TABLE IF NOT EXISTS ${hiveconf:__GREEN_MAIN_DB__}.emi_t_supvr_rpt_to_hrchy(
    `clnt_obj_id` STRING,
    `pers_obj_id` STRING,
...
INSERT OVERWRITE TABLE ${hiveconf:__GREEN_MAIN_DB__}.emi_t_supvr_rpt_to_hrchy PARTITION(environment)
SELECT
    /*+ COALESCE(800) */
    ext.clnt_obj_id,
    ...
    ext.environment
FROM
    ${hiveconf:__BLUE_RAW_DB__}.dwh_t_supvr_rpt_to_hrchy ext
    --LEFT OUTER JOIN (
    --    SELECT
    --        clnt_obj_id,
