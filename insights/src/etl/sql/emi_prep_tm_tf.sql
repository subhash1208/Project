...
CREATE TABLE IF NOT EXISTS ${__BLUE_MAIN_DB__}.emi_prep_tm_tf (
    clnt_obj_id STRING, 
    pers_obj_id STRING,
...
--DROP TABLE IF EXISTS ${__GREEN_MAIN_DB__}.tmp_emi_clnt_last_load_yr_tm_tf;
--
--CREATE TABLE ${__GREEN_MAIN_DB__}.tmp_emi_clnt_last_load_yr_tm_tf
--USING PARQUET
--AS
--SELECT environment, min(yr_cd) as last_load_clndr_yr_cd
--FROM 
--    (SELECT environment, clnt_obj_id, max(yr_cd) as yr_cd 
--    FROM ${__GREEN_MAIN_DB__}.emi_prep_tm_tf 
--    --WHERE environment = '${environment}'
--    GROUP BY environment, clnt_obj_id) IGN
--GROUP BY environment;

INSERT OVERWRITE TABLE ${__BLUE_MAIN_DB__}.emi_prep_tm_tf PARTITION(environment)
SELECT 
       ...
       ftw.prem_tm_ind,
       ftw.paid_tm_off_ind,
       ftw.plan_absence_ind,
       ftw.reg_temp_cd,
       ftw.paycd_clsfn_cd,
       ftw.hr_orgn_id,
       ftw.payrl_orgn_id,
       ftw.pay_rt_type_cd,
       CASE WHEN ftw.actl_hrs_in_secnd_nbr IS NOT NULL THEN ftw.actl_hrs_in_secnd_nbr END AS actl_hrs_in_secnd_nbr,
       CASE WHEN (ftw.clnt_cnfrm_ind = 1 AND ftw.ot_ind = 1 and ftw.actl_hrs_in_secnd_nbr IS NOT NULL) THEN ftw.actl_hrs_in_secnd_nbr END AS ot_hrs_in_secnd,
       CASE WHEN (ftw.clnt_cnfrm_ind = 1 AND ftw.absence_ind = 1 and ftw.actl_hrs_in_secnd_nbr IS NOT NULL) THEN ftw.actl_hrs_in_secnd_nbr END AS absence_hrs_in_secnd,
       ftw.time_product,
       ftw.clndr_wk_cd,
       ftw.trans_dt,
       ftw.db_schema,
       ftw.yr_cd,
       ftw.environment
  FROM ${__BLUE_MAIN_DB__}.emi_base_tm_tf ftw
 --LEFT OUTER JOIN ${__GREEN_MAIN_DB__}.tmp_emi_clnt_last_load_yr_tm_tf t
