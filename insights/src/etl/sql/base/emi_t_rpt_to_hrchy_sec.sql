...
CREATE TABLE IF NOT EXISTS ${__BLUE_MAIN_DB__}.emi_t_rpt_to_hrchy_sec(
    `clnt_obj_id` STRING,
    `pers_obj_id` STRING,
...
INSERT OVERWRITE TABLE ${__BLUE_MAIN_DB__}.emi_t_rpt_to_hrchy_sec PARTITION(environment)
SELECT
    /*+ COALESCE(800) */
    ext.clnt_obj_id,
...
WHERE
    ...
    AND (ext.clnt_live_ind = 'Y' OR ext.clnt_obj_id IN (select distinct clnt_obj_id from ${__BLUE_MAIN_DB__}.emi_non_live_clnts));