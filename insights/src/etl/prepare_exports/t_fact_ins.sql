...
CREATE TABLE IF NOT EXISTS ${__BLUE_MAIN_DB__}.t_fact_ins (
    ins_hash_val string,
    clnt_obj_id string,
    clnt_obj_id_cmpr_with string,
    yr_cd string,
...
ON ins.ins_hash_val = t_fact.ins_hash_val
 --LEFT OUTER JOIN (SELECT * FROM ${__GREEN_MAIN_DB__}.t_fact_insights_archive
--    WHERE '${incremental}' = 'true') arch
...
--LEFT OUTER JOIN ${__GREEN_MAIN_DB__}.clnt_min_max_mtrc_summ summ
...
INSERT OVERWRITE TABLE ${__BLUE_MAIN_DB__}.t_fact_ins PARTITION(environment)
SELECT
    ins_hash_val,
    clnt_obj_id,
    clnt_obj_id_cmpr_with,
